...
const GamingElectronicsData = [
  { id: 1, image: "/images/ps5.jpg", title: "PlayStation 5", brand: "Sony", color: "White", price: 500 },
  { id: 2, image: "/images/xbox.jpg", title: "Xbox Series X", brand: "Microsoft", color: "Black", price: 480 },
  { id: 3, image: "/images/keyboardgaming.jpg", title: "Gaming Keyboard", brand: "Razer", color: "Black", price: 120 },
];

export default function GamingElectronics() {
  ...
  const filteredProducts = GamingElectronicsData.filter(product =>
      ...
      <div className="px-6 py-10 grid grid-cols-1 md:grid-cols-4 gap-6">
        <aside className="bg-white shadow rounded-lg p-4">
          ...
          <select className="w-full border rounded p-2 mb-4" value={brand} onChange={(e)=>setBrand(e.target.value)}>
            <option>All</option>
            {[...new Set(GamingElectronicsData.map(p=>p.brand))].map((b,i)=><option key={i}>{b}</option>)}
          </select>

          <label className="block mb-2">Color</label>
          <select className="w-full border rounded p-2 mb-4" value={color} onChange={(e)=>setColor(e.target.value)}>
            <option>All</option>
            {[...new Set(GamingElectronicsData.map(p=>p.color))].map((c,i)=><option key={i}>{c}</option>)}
          </select>
