...
const GroceriesData = [
  { id: 1, image: "/images/rice.jpg", title: "Rice Pack", brand: "Golden Grain", category: "Food", price: 25 },
  { id: 2, image: "/images/oil.jpg", title: "Cooking Oil", brand: "SunGold", category: "Food", price: 15 },
  { id: 3, image: "/images/stationery.jpg", title: "Office Stationery Set", brand: "Staples", category: "Corporate", price: 40 },
];

export default function GroceriesCorporates() {
      ...
      <div className="px-6 py-10 grid grid-cols-1 md:grid-cols-4 gap-6">
        <aside className="bg-white shadow rounded-lg p-4">
          ...
          <select className="w-full border rounded p-2 mb-4" value={brand} onChange={(e)=>setBrand(e.target.value)}>
            <option>All</option>
            {[...new Set(GroceriesData.map(p=>p.brand))].map((b,i)=><option key={i}>{b}</option>)}
          </select>

          <label className="block mb-2">Category</label>
          <select className="w-full border rounded p-2 mb-4" value={category} onChange={(e)=>setCategory(e.target.value)}>
            <option>All</option>
            {[...new Set(GroceriesData.map(p=>p.category))].map((c,i)=><option key={i}>{c}</option>)}
          </select>
