...
const MobilePhonesData = [
  { id: 1, image: "/images/iphone13.jpg", title: "iPhone 13", brand: "Apple", color: "White", price: 900 },
  { id: 2, image: "/images/iphone14.jpg", title: "iPhone 14", brand: "Apple", color: "Black", price: 1100 },
  { id: 3, image: "/images/galaxys22.jpg", title: "Galaxy S22", brand: "Samsung", color: "Silver", price: 850 },
];
...
export default function MobilePhones() {
  ...
  const filteredProducts = MobilePhonesData.filter(product =>
      ...
      <div className="px-6 py-10 grid grid-cols-1 md:grid-cols-4 gap-6">
        <aside className="bg-white shadow rounded-lg p-4">
          ...
          <select className="w-full border rounded p-2 mb-4" value={brand} onChange={(e)=>setBrand(e.target.value)}>
            <option>All</option>
            {[...new Set(MobilePhonesData.map(p=>p.brand))].map((b,i)=><option key={i}>{b}</option>)}
          </select>

          <label className="block mb-2">Color</label>
          <select className="w-full border rounded p-2 mb-4" value={color} onChange={(e)=>setColor(e.target.value)}>
            <option>All</option>
            {[...new Set(MobilePhonesData.map(p=>p.color))].map((c,i)=><option key={i}>{c}</option>)}
          </select>
